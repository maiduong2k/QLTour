...
//        plBtn.add(btnSua);
    ...
    private void btnSuaMouseClicked() {
        String mat = formHienThi.getSelectedRow(2);
        ThemSuaChiTietHoaDonForm themcthd = new ThemSuaChiTietHoaDonForm("Sửa", this.mahd, mat);
    ...
    private void btnXoaMouseClicked() {
        String mat = formHienThi.getSelectedRow(1);
        String soluong = formHienThi.getSelectedRow(2);
        if (mat != null) {
            if (JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa " + soluong + " tour " + mat + " của hóa đơn "+ this.mahd +"?", "Chú ý", JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
                new QuanLyChiTietHoaDonBUS().delete(this.mahd, mat);
                formHienThi.refresh();
            }

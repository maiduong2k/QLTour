...
public class QuanLyChiTietPhieuNhapDAO {
    ...
    public boolean add(ChiTietPhieuNhap ctpn) {
        qlctpnConnection = new ConnectionDB();
        Boolean ok = qlctpnConnection.sqlUpdate("INSERT INTO `chitietphieunhap`(`MaPN`,`MaT`,`SoLuong`,`DonGia`) VALUE('"
                + ctpn.getMa() + "', '" + ctpn.getMaT() + "','" + ctpn.getSoLuong() + "','" + ctpn.getDonGia() + "')");
        qlctpnConnection.closeConnect();
        return ok;
    ...
    public Boolean delete(String _mapn, String _masp) {
        qlctpnConnection = new ConnectionDB();
        Boolean success = qlctpnConnection.sqlUpdate("DELETE FROM chitietphieunhap WHERE MaPN='" + _mapn + "' AND MaT='" + _masp + "';");
        qlctpnConnection.closeConnect();
        return success;
    }

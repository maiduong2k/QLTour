...
public class QuanLyPhieuNhapBUS {
        ...
        dspn.forEach((pn) -> {
            if (type.equals("Tất cả")) {
                if (pn.getMaPN().toLowerCase().contains(value.toLowerCase())
                        || pn.getMaKS().toLowerCase().contains(value.toLowerCase())
                ...
                switch (type) {
                    case "Mã phiếu nhập":
                        ...
                        break;
                    case "Mã khách sạn":
                        if (pn.getMaKS().toLowerCase().contains(value.toLowerCase())) {
                            result.add(pn);
                        }
                        break;
                    case "Mã nhân viên":
    ...
    public Boolean update(String maPN, String maKS, String maNV, LocalDate ngayNhap, LocalTime gioNhap, float tongTien) {
        PhieuNhap pn = new PhieuNhap(maPN, maKS, maNV, ngayNhap, gioNhap, tongTien);
        return update(pn);
    }
    ...
    public boolean add(String maPN, String maKS, String maNV, LocalDate ngayNhap, LocalTime gioNhap, float tongTien) {
        PhieuNhap pn = new PhieuNhap(maPN, maKS, maNV, ngayNhap, gioNhap, tongTien);
        return add(pn);
    }

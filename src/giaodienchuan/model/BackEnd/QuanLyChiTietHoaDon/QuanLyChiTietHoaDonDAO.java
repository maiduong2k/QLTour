...
public class QuanLyChiTietHoaDonDAO {
    ...
    public ArrayList readDB() {
        ...
        try {
            ...
            if (rs != null) {
                while (rs.next()) {
                    ChiTietHoaDon hd = new ChiTietHoaDon(rs.getString("MaHD"), rs.getString("MaT"), rs.getInt("SoLuong"), rs.getFloat("DonGia"));
                    dshd.add(hd);
                }
            }
    ...
    public Boolean add(ChiTietHoaDon hd) {
        connection = new ConnectionDB();
        Boolean success = connection.sqlUpdate("INSERT INTO chitiethoadon(MaHD,MaT,SoLuong,DonGia) VALUES ('" 
                + hd.getMaHoaDon() + "','" 
                + hd.getMaTour() + "','" 
                + hd.getSoLuong() + "','" 
                + hd.getDonGia() + "');");
    ...
    public Boolean update(ChiTietHoaDon ct) {
        ...
        Boolean success = connection.sqlUpdate("UPDATE chitiethoadon set "
                ...
                + "', DonGia='" + ct.getDonGia()
                + "' WHERE MaHD='" + ct.getMaHoaDon() + "' AND MaT='" + ct.getMaTour() + "';");
        connection.closeConnect();
        return success;
    }
